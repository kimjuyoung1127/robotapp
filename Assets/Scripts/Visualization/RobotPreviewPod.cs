    ...
    public class RobotPreviewPod : MonoBehaviour
    {
        ...
        private bool TryCacheNamedAxisChain()
        {
            ...
            for (var i = 0; i < targetPaths.Length; i++)
            {
                ...
                targets[i] = target;
                if (TryReadDriveSpec(target, templateSpecs, i, out var axis, out var spec))
                {
                    axes[i] = axis;
                    specs[i] = spec;
                }
        ...
        private static bool TryReadDriveSpec(Transform target, JointControlSpec[] templateSpecs, int jointIndex, out Vector3 axis, out JointControlSpec spec)
        {
            axis = Vector3.forward;
            spec = jointIndex < templateSpecs.Length
                ? templateSpecs[jointIndex]
                : new JointControlSpec(-180f, 180f, true);

            var driveComponent = FindComponentByTypeName(target, "realvirtual.Drive");
            if (driveComponent == null)
            {
                return false;
            }

            var directionValue = ReadFieldValue(driveComponent, "Direction");
            var directionName = directionValue != null ? directionValue.ToString() : string.Empty;
            var reverseDirection = ReadBoolField(driveComponent, "ReverseDirection");
            var isRotation = ReadBoolField(driveComponent, "IsRotation");
            var useLimits = ReadBoolField(driveComponent, "UseLimits");
            var lowerLimit = ReadFloatField(driveComponent, "LowerLimit");
            var upperLimit = ReadFloatField(driveComponent, "UpperLimit");

            axis = DirectionNameToVector(directionName);
            if (reverseDirection)
            {
                axis = -axis;
            }
            ...
            if (useLimits)
            {
                spec = new JointControlSpec(lowerLimit, upperLimit, isRotation);
            }
            else
            {
                spec = new JointControlSpec(spec.Min, spec.Max, isRotation ? spec.IsRotational : false);
            }

            return true;
        }

        private static Component FindComponentByTypeName(Transform target, string fullTypeName)
        {
            if (target == null)
            {
                return null;
        ...
        private static object ReadFieldValue(Component component, string fieldName)
        {
            if (component == null)
            {
                return null;
        ...
        private static bool ReadBoolField(Component component, string fieldName)
        {
            var value = ReadFieldValue(component, fieldName);
            return value is bool boolValue && boolValue;
        }

        private static float ReadFloatField(Component component, string fieldName)
        {
            var value = ReadFieldValue(component, fieldName);
            return value is float floatValue ? floatValue : 0f;
        }

        private static Vector3 DirectionNameToVector(string directionName)
        {
