...
- 외부 범용 하네스는 이 저장소의 SSOT가 아니라 참고원이다.
- 흡수 대상은 운영 거버넌스 문구, 승격 규칙, 체크리스트 템플릿처럼 프로젝트 비의존적인 내용으로 제한한다.
- 로딩 순서, Unity 전용 검증 루프, 폴더 책임, `unityctl` 운영 방식은 계속 이 문서와 로컬 `AGENTS.md` / `CLAUDE.md`가 정한다.
- 범용 템플릿을 가져올 때도 현재 저장소의 명명, 경계, 검증 습관에 맞게 재해석해서 반영한다.
- 새 규칙을 추가할 때는 "이 프로젝트에서 실제로 반복되었는가"를 먼저 확인하고, 유효성이 검증된 뒤에만 로컬 하네스로 승격한다.
