...
- `묶음` 탭은 선택한 포인트 참조를 묶는 기능이며, 누락 참조는 `시퀀스/포인트` 목록 상태와 함께 보여준다.
- 탭 순서는 `포인트 / 묶음 / 시퀀스`다.
...
- 묶음 row UX 기준:
  - 묶음 탭 상단 요약은 묶음 개수, 선택 개수, 선택 묶음의 참조/누락 정보를 보여준다.
  - 묶음 row의 `선택` 버튼으로 여러 묶음을 선택할 수 있다.
  - 일괄 작업은 `선택 해제`, `선택 복사`, `선택 삭제`, `전체 삭제`를 제공한다.
  - `전체 삭제`는 confirmation 2회 클릭으로만 실행한다.
  - 포인트를 묶음 후보에 추가하면 `[Bundle] 후보 추가` feedback을 보여줘 사용자가 완료를 알 수 있어야 한다.
  - 전체 삭제 후에도 새 포인트 후보를 추가해 새 묶음을 만들고 바로 DryRun 실행할 수 있어야 한다.
...
- 포인트 row UX 기준:
  ...
  - 일괄 작업은 `선택 해제`, `선택 속도 저장`, `선택 묶음 추가`, `선택 삭제`를 제공한다.
