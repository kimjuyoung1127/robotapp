...
- 함수 row UX 기준:
  - 함수 탭 상단 요약은 함수 개수, 선택 개수, 선택 함수의 참조/누락 정보를 보여준다.
  - 함수 row의 `선택` 버튼으로 여러 함수를 선택할 수 있다.
  - 일괄 작업은 `선택 해제`, `선택 복사`, `선택 삭제`를 제공한다.
  - `버튼 접기`는 함수 row를 `선택 / 열기` 중심으로 줄인다.
- 포인트 row UX 기준:
  - row 전체 클릭은 선택/상세 확인만 한다.
  - row의 `선택` 버튼으로 여러 포인트를 선택할 수 있다.
  - row 버튼은 `실행`, `미리보기`, `편집`, `함수 추가`로 표시한다.
  ...
  - 일괄 작업은 `선택 해제`, `선택 속도 저장`, `선택 함수 추가`, `선택 삭제`를 제공한다.
